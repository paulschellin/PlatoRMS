...
#define SPLIT_SAVE_LOAD 1

/*
	Because we want to export to XML, the name-value pair macro will be
	useful (BOOST_SERIALIZE_NVP()).

	It might be necessary to provide the macro BOOST_CLASS_VERSION(T,0) as
	well.

	Split save and load functions are probably necessary for more complicated
	classes, especially those which have pointers and/or references.

	That may need the BOOST_SERIALIZATION_SPLIT_MEMBER() macro under the split
	definitions, or it might need more careful prototyping. The following two
	links are somewhat conflicting:

	http://www.boost.org/doc/libs/1_55_0/libs/serialization/doc/tutorial.html

	http://stackoverflow.com/a/5945097/2617340



	Most Useful!

	http://forums.codeguru.com/showthread.php?415510-boost-serialization-problem-with-reference-members
 */
...
class TagDef;
std::ostream& operator<<(std::ostream& os, const TagDef& object);

class TagDef {
//#ifndef SPLIT_SAVE_LOAD
  private:
...
std::ostream& operator<<(std::ostream& os, const TagDef& object)
{
	os << boost::format("%1% %|20t|%2% %|20t|%3%") % object.name % object.type % object.description;
	return os;
}

BOOST_CLASS_VERSION(TagDef, 0)

/*
#ifdef SPLIT_SAVE_LOAD
namespace boost { namespace serialization {
template<class Archive>
inline void save_construct_data(
	Archive & ar, const TagDef* obj, const unsigned int //file_version
) {
	ar << obj->name;
	ar << obj->type;
	ar << obj->description;
...
inline void load_construct_data(
	Archive & ar, TagDef* obj, const unsigned int //file_version
) {
	std::string name_in;
	ar >> name_in;
	...
	::new(obj)TagDef(name_in, type_in, desc_in);
}
}}	//	namespace
#endif
*/
...
template<class Archive>
inline void save_construct_data(
	Archive & ar, const File* obj, const unsigned int //file_version
) {
	ar << obj->name;
	ar << obj->path;
}
...
template<class Archive>
inline void load_construct_data(
	Archive & ar, File* obj, const unsigned int //file_version
) {
	std::string name_in;
	ar >> name_in;
...
//#endif
  public:
	...
	const TagDef& getConstTagDefRefMember(void) const
	{ return tag; }

	const File& getConstFileRefMember(void) const
	{ return file; }

	const std::string& getConstStringMember(void) const
	{ return value; }
...
template<class Archive>
inline void save_construct_data(
	Archive & ar, const TagInstance* obj, const unsigned int //file_version
) {
	ar << boost::serialization::make_nvp("tag", obj->getConstTagDefRefMember());
	ar << boost::serialization::make_nvp("file", obj->getConstFileRefMember());
	ar << boost::serialization::make_nvp("value", obj->getConstStringMember());
}

template<class Archive>
inline void load_construct_data(
	Archive & ar, TagInstance* obj, const unsigned int //file_version
) {
	/*
	TagDef* tag_in;
	ar >> tag_in;
	...
	*/
	...
	TagDef* tag_ptr_in = new TagDef();
	File*	file_ptr_in = new File();
	std::string value_in;

	ar >> boost::serialization::make_nvp("tag", *tag_ptr_in);
	ar >> boost::serialization::make_nvp("file", *file_ptr_in);
	ar >> boost::serialization::make_nvp("value", value_in);

	::new(obj)TagInstance(*tag_ptr_in, *file_ptr_in, value_in);
}
}}	//	namespace
//#endif
...
/*
	...
	oa << BOOST_SERIALIZATION_NVP(ti);

			...becomes...

	TagInstance ti;
	...(do stuff with ti)...
 	TagInstance* ti_ptr = &ti;
	boost::archive::xml_oarchive oa(ofs);
	oa << BOOST_SERIALIZATION_NVP(ti_ptr);


				and

	TagInstance* ti_ptr = NULL;
	boost::archive::xml_iarchive ia(ifs);
	ia >> BOOST_SERIALIZATION_NVP(*ti_ptr);
...
template <typename T>
void
deserialize_from_xml (T& obj, std::string if_name)
{
	T* obj_ptr;
	fs::path if_path(fs::canonical(fs::absolute(fs::path(if_name))));
	std::ifstream ifs (if_path.string());
	assert(ifs.good());

	boost::archive::xml_iarchive ia (ifs);
	//T obj;
	ia >> BOOST_SERIALIZATION_NVP(obj_ptr);

	//std::cout << *obj_ptr;
	//obj = *obj_ptr;

	delete &(obj_ptr->getConstTagDefRefMember());
	delete &(obj_ptr->getConstFileRefMember());
	delete obj_ptr;
}
...
/*
	//decltype(tags) in_tags;
	std::vector< TagDef > in_tags;
	{
		std::ifstream ifs (std::string("tags_") + varMap["output-file"].as<std::string>());
		assert (ifs.good());
		boost::archive::xml_iarchive ia (ifs);
...
*/
	...
	//decltype(tags) in_tags (deserialize_from_xml<decltype(tags)>(std::string("tags_") + varMap["output-file"].as<std::string>()));
	//decltype(files) in_files (deserialize_from_xml<decltype(tags)>(std::string("files_") + varMap["output-file"].as<std::string>()));
	//decltype(tags) in_tagSet (deserialize_from_xml(std::string("tagSet_") + varMap["output-file"].as<std::string>()));
